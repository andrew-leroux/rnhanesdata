...
In what follows, we compare Net reclassification Index (NRI) for the two models:
...
#library(rnicens)
library(PredictABEL)
reclassification(data = data_analysis, 
                 cOutcome=which(names(data_analysis) == "yr5_mort"), 
                 predrisk1 = predict(fit_svy_noPA, type = "response"), #risks of the non-activity based model
                 predrisk2 = predict(fit_final, type = "response"), #risks for our final model
                 cutoff = c(0, 0.3, 0.5, 1))
```

# Comparison with the best forward selction no-PA model

```{r, message = FALSE, results=FALSE, eval = TRUE }
...
```{r, , message = FALSE, results=FALSE, eval = run_auc_noPA }
ind_vars <- c("Age", "Gender", "Race", "EducationAdult", "SmokeCigs", "DrinkStatus", "BMI_cat",
              "Diabetes","CHF",  "CHD", "Stroke", "Cancer", "MobilityProblem",
              "sPC6", "ST", "WT", "MVPA","TAC", "TLAC", "SATP", "ASTP", paste0("TLAC_",c(1:12)))
