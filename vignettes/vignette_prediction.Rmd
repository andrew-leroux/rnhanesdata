...
if (!dir.exists(dir_tmp)){
  ...
  dl_file = function(url) {
    ...
    if (!file.exists(destfile)) {
      out = download.file(url, destfile = destfile)
    }
...
bout_mat = matrix(nrow = nrow(act_mat), ncol = 2)
...
flag_mat <- AllFlags[,paste0("MIN",1:1440)]
...
drop_cn = c(paste0("MIN",1:1440), "TAC","TLAC","WT","ST","MVPA", 
   ...
   paste0("TLAC_",1:12))
table_dat <- AllAct[!duplicated(AllAct$SEQN), setdiff(cn, drop_cn)]
